...
const EXPECTED_SEQUENCE = ['play', 'mute'];

async function runGate(options = {}) {
  const base = await runBoundedGate({
    ...options,
    expectedAction: 'play',
    expectedTitle: options.expectedTitle || 'MUTE PASS'
  });
  ...
  return {
    ...
    gate: 'offline-strategy-history-multistep',
...
async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    const result = await runGate({
      ...
      expectedTitle: args['expected-title'] || 'MUTE PASS',
