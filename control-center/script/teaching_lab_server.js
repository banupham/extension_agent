...
const MOTOR_SCENARIOS = Object.freeze({
  ...
  M10: { family: 'KEYBOARD', title: 'Enter submit', task: 'Nhập Agent vào Keyword rồi nhấn Enter.', expected: 'Search Submitted', type: 'motor-enter' },
  M11: { family: 'KEYBOARD', title: 'Tab traversal', task: 'Từ ô Email đang có sẵn, nhấn Tab sang Message, nhập Ready vào Message rồi gửi.', expected: 'Message Sent', type: 'motor-tab-form' },
  ...
  M16: { family: 'COMPOSITE', title: 'Scroll form composite', task: 'Tìm Project Delta ở cuối trang, mở nó, nhập Ready for review vào Note, bật Approved rồi gửi.', expected: 'Project Submitted', type: 'motor-composite-project' },
  M17: { family: 'COMPOSITE', title: 'Menu deployment composite', task: 'Trong menu Operations, mở Deployment, chọn Production rồi kéo Priority sang vùng High và lưu.', expected: 'Deployment Saved', type: 'motor-composite-deployment' },
...
<style>
body{font:16px system-ui,sans-serif;margin:0;background:#f5f7fb;color:#172033}main{max-width:900px;margin:32px auto;padding:0 20px}.card{background:#fff;border:1px solid #d9deea;border-radius:14px;padding:22px}.meta{font-size:13px;color:#667085}.task{padding:13px;background:#eef4ff;border-radius:10px;margin:14px 0}.stage{position:relative;min-height:280px;border:1px dashed #b8c0cf;border-radius:12px;padding:22px;overflow:auto}.row{display:flex;gap:14px;flex-wrap:wrap}.column{display:flex;flex-direction:column;gap:12px}.panel{padding:14px;border:1px solid #d0d5dd;border-radius:10px;background:#fff}button,input,select,textarea{font:inherit;padding:10px 12px;border:1px solid #98a2b3;border-radius:8px}button{background:#fff;cursor:pointer}.primary{background:#175cd3;color:#fff;border-color:#175cd3}.muted{color:#667085}.result{margin-top:18px;padding:12px;border-radius:9px;background:#ecfdf3;color:#027a48;font-weight:700;display:none}.moving{position:absolute;transition:left .18s linear}.dropzone{min-height:120px;border:2px dashed #98a2b3;border-radius:12px;padding:16px;min-width:180px}.dragitem{display:inline-block;padding:12px 16px;background:#eef4ff;border:1px solid #84adff;border-radius:9px;cursor:grab}.hscroll{display:flex;gap:18px;width:1250px}.hviewport{overflow-x:auto;border:1px solid #d0d5dd;border-radius:10px;padding:16px}.hcard{min-width:230px;height:100px;display:grid;place-items:center;border:1px solid #d0d5dd;border-radius:10px;background:#fff}.menu{position:relative;display:inline-block}.submenu{display:none;position:absolute;left:0;top:44px;z-index:3;padding:8px;background:#fff;border:1px solid #d0d5dd;border-radius:8px}.menu.open .submenu{display:block}a{color:#175cd3}.spacer{height:680px}.media-panel{display:flex;gap:12px;align-items:center;flex-wrap:wrap}.media-panel label{display:flex;gap:8px;align-items:center}.media-panel input[type=range]{width:220px}
</style></head>
...
function renderMotorScenario(id, s) {
  ...
  if (s.type === 'motor-tab-form') return layout(id, s, `<form id="form" class="column"><label>Email <input id="email" type="email" aria-label="Email" value="demo@example.test" autocomplete="off"></label><label>Message <textarea id="message" aria-label="Message"></textarea></label><button id="send" class="primary" type="submit">Send</button></form>`, `let tabReached=false;message.onfocus=()=>{tabReached=true;};form.onsubmit=e=>{e.preventDefault();if(message.value==='Ready'&&tabReached)success(${done});};`);
  if (s.type === 'motor-select-check') return layout(id, s, `<div class="column"><label>Car <select id="car" aria-label="Car"><option>BMW</option><option>Audi</option><option>Volvo</option></select></label><label><input id="warranty" type="checkbox" aria-label="Warranty"> Warranty</label><button id="save" class="primary">Save</button></div>`, `save.onclick=()=>{if(car.value==='Audi'&&warranty.checked)success(${done});};`);
  if (s.type === 'motor-toggle') return layout(id, s, `<div class="column"><label><input id="notifications" type="checkbox" aria-label="Notifications" checked> Notifications</label><label><input id="dark" type="checkbox" aria-label="Dark Mode"> Dark Mode</label></div>`, `function check(){if(!notifications.checked&&dark.checked)success(${done});}notifications.onchange=check;dark.onchange=check;`);
  ...
  if (s.type === 'motor-media-basic') return layout(id, s, `<audio id="media" aria-label="Training Media" hidden></audio><div class="media-panel"><button id="play">Play</button><button id="mute">Mute</button><label>Seek <input id="seek" type="range" aria-label="Seek" min="0" max="100" step="1" value="0"></label><button id="pause">Pause</button></div><p class="muted">Media cục bộ 6 giây; các control đều có semantic target riêng.</p>`, `makeTone(media);let played=false,muted=false,seeked=false;play.onclick=()=>{played=true;media.play().catch(()=>{});};mute.onclick=()=>{media.muted=true;muted=true;};seek.oninput=()=>{media.currentTime=Number(seek.value)*.06;if(Number(seek.value)>=45)seeked=true;};pause.onclick=()=>{media.pause();if(played&&muted&&seeked)success(${done});};`);
  if (s.type === 'motor-composite-project') return layout(id, s, `<div class="spacer"><p class="muted">Project Delta nằm ở cuối.</p></div><div class="panel"><button id="open">Project Delta</button><div id="projectForm" hidden class="column"><label>Note <input id="note" aria-label="Note" autocomplete="off"></label><label><input id="approved" type="checkbox" aria-label="Approved"> Approved</label><button id="submit" class="primary">Submit</button></div></div>`, `open.onclick=()=>projectForm.hidden=false;submit.onclick=()=>{if(note.value==='Ready for review'&&approved.checked)success(${done});};`);
  if (s.type === 'motor-composite-deployment') return layout(id, s, `<div id="ops" class="menu"><button>Operations</button><div class="submenu"><button id="deployment">Deployment</button></div></div><div id="deployForm" class="column panel" hidden><label>Environment <select id="env" aria-label="Environment"><option>Staging</option><option>Production</option></select></label><div class="row"><div id="priority" class="dragitem" draggable="true">Priority</div><div id="high" class="dropzone" role="region" aria-label="High Priority zone">High</div></div><button id="save" class="primary">Save</button></div>`, `let priorityHigh=false;ops.onmouseenter=()=>ops.classList.add('open');deployment.onclick=()=>deployForm.hidden=false;priority.ondragstart=e=>e.dataTransfer.setData('text/plain','Priority');high.ondragover=e=>e.preventDefault();high.ondrop=e=>{e.preventDefault();high.appendChild(priority);priorityHigh=true;};save.onclick=()=>{if(env.value==='Production'&&priorityHigh)success(${done});};`);
  ...
  if (s.type === 'motor-media-advanced') return layout(id, s, `<audio id="media" aria-label="Advanced Training Media" hidden></audio><div class="column"><div class="media-panel"><button id="play">Play</button><label>Playback Rate <select id="rate" aria-label="Playback Rate"><option value="1">1x</option><option value="2">2x</option></select></label><label>Volume <input id="volume" type="range" aria-label="Volume" min="0" max="100" step="1" value="100"></label><label>Seek <input id="seek" type="range" aria-label="Seek" min="0" max="100" step="1" value="0"></label></div><div class="media-panel"><button id="mute">Mute</button><button id="unmute">Unmute</button><button id="pause">Pause</button></div></div>`, `makeTone(media);let played=false,rateDone=false,volumeDone=false,seekDone=false,muted=false,unmuted=false;play.onclick=()=>{played=true;media.play().catch(()=>{});};rate.onchange=()=>{media.playbackRate=Number(rate.value);if(Number(rate.value)===2)rateDone=true;};volume.oninput=()=>{media.volume=Number(volume.value)/100;if(Number(volume.value)===70)volumeDone=true;};seek.oninput=()=>{media.currentTime=Number(seek.value)*.06;if(Number(seek.value)>=45&&Number(seek.value)<=55)seekDone=true;};mute.onclick=()=>{media.muted=true;muted=true;};unmute.onclick=()=>{media.muted=false;if(muted)unmuted=true;};pause.onclick=()=>{media.pause();if(played&&rateDone&&volumeDone&&seekDone&&muted&&unmuted)success(${done});};`);
...
function runSelfTest() {
  ...
  assert.ok(renderMotorScenario('M03', MOTOR_SCENARIOS.M03).includes("tools.classList.add('open')"));
  assert.ok(renderMotorScenario('M04', MOTOR_SCENARIOS.M04).includes('role="region" aria-label="Completed drop zone"'));
  assert.ok(renderMotorScenario('M11', MOTOR_SCENARIOS.M11).includes('value="demo@example.test"'));
  assert.ok(renderMotorScenario('M15', MOTOR_SCENARIOS.M15).includes('aria-label="Seek"'));
  assert.ok(renderMotorScenario('M17', MOTOR_SCENARIOS.M17).includes('High Priority zone'));
  assert.ok(renderMotorScenario('M21', MOTOR_SCENARIOS.M21).includes('aria-label="Playback Rate"'));
  assert.ok(renderMotorScenario('M21', MOTOR_SCENARIOS.M21).includes('aria-label="Volume"'));
