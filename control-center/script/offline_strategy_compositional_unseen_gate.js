...
async function runGate(options = {}) {
  const base = await runBoundedGate({
    ...options,
    expectedAction: 'play',
    expectedTitle: options.expectedTitle || 'PAUSE PASS'
  });
  ...
  if (base.steps?.[0]?.decision?.metadata?.prototypeSource && base.steps[0].decision.metadata.prototypeSource !== 'taskComposition') {
    errors.push(`step0_source:${base.steps[0].decision.metadata.prototypeSource}`);
  }
  ...
  if (base.steps?.[1]?.outcome?.taskSucceeded !== true) errors.push('step1_should_finish_task');
  ...
  if (base.invariant?.actionExecutionCount !== 2) errors.push(`action_execution_count:${base.invariant?.actionExecutionCount}`);
  if (base.invariant?.strategyCallCount !== 2) errors.push(`strategy_call_count:${base.invariant?.strategyCallCount}`);

  return {
    ...base,
    ok: errors.length === 0,
    result: errors.length === 0 ? 'PASS' : 'FAIL',
    gate: 'offline-strategy-compositional-unseen',
...
async function main(argv = process.argv.slice(2)) {
  ...
  const instruction = args.task || 'Play the media, then pause it';
