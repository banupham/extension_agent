...
assert.ok(background.includes("'core/task_episode_review_export.js'"));
...
assert.ok(background.includes('TASK_REVIEW_OUTBOX_KEY'), 'successful Task Episode reviews need a durable extension outbox');
assert.ok(background.includes('TaskEpisodeReviewExport.buildReviewExport'), 'Mark Success must build the privacy-safe review automatically');
assert.ok(background.includes('await persistTaskReview(review)'), 'review must persist before socket delivery');
assert.ok(background.includes('SocketMirror?.registerTaskReview?.(review)'), 'persisted review must enter the existing socket mirror');
assert.ok(background.indexOf('await persistTaskReview(review)') < background.indexOf('SocketMirror?.registerTaskReview?.(review)'));
assert.ok(background.includes('registerTaskReviewBacklog'), 'service-worker restart must replay pending Task Episode reviews');
...
assert.ok(mirror.includes("type: 'task-episode-review'"));
assert.ok(mirror.includes("message?.type === 'task-episode-review-ack'"));
assert.ok(mirror.includes("message?.type === 'pipeline-status'"));
assert.ok(mirror.includes("type: 'pipeline-status-request'"));
assert.ok(mirror.includes('registerTaskReview'));
assert.ok(mirror.includes('taskReviewOutboxCount'));
...
assert.ok(server.includes('receiveTaskReview'), 'socket server must receive Task Episode reviews on the existing endpoint');
assert.ok(server.includes('reviewSafety'), 'Task Episode transport must fail closed on privacy flags');
assert.ok(server.includes('scheduleReviewProcessing'), 'persisted reviews must enter automatic machine processing');
assert.ok(server.includes('prepareIncrementalStrategyLearning'), 'automatic processing must reuse the existing incremental orchestrator');
assert.ok(server.includes('finalizeMachineAcceptedStrategyLearning'), 'only machine ACCEPT batches may finalize a candidate');
assert.ok(server.includes('protectFinalizedCandidate'), 'candidate protection must reuse the existing protection runner');
assert.ok(server.includes('PIPELINE_BATCH_THRESHOLD'), 'candidate creation must be batch-gated');
assert.ok(server.includes('productionPromotionAllowed: false'), 'automatic backend must never unlock production promotion');
assert.ok(server.includes('promotionApplied: false'), 'candidate state must explicitly record no automatic promotion');
...
assert.ok(popup.includes('showPipeline'));
assert.ok(popup.includes('Candidate buffer:'));
assert.ok(popupHtml.includes('Strategy Pipeline Monitor'));
...
console.log(`Training Collector V0.8 socket + Task Episode pipeline inheritance contract OK (collector ${manifest.version})`);