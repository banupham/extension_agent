...
const raw = {
  ...
  events: [
    ...
    { type: 'dom-click', tsEpochMs: 1000, sessionSeq: 7, pageSeq: 2, sourceSeq: 1, pageInstanceId: 'p1', tabId: 1, frameId: 0, targetRef: 'eChild', resolvedTargetRef: 'eButton', resolvedTarget: { tag: 'button', role: 'button', label: 'Thích', rect: { x: 10, y: 20, width: 80, height: 30 } } },
    ...
    { type: 'dom-hover-enter', tsEpochMs: 4000, sessionSeq: 40, pageSeq: 11, sourceSeq: 1, pageInstanceId: 'p1', targetRef: 'thumb' },
    { type: 'dom-hover-dwell', tsEpochMs: 4400, sessionSeq: 41, pageSeq: 12, sourceSeq: 2, pageInstanceId: 'p1', targetRef: 'thumb', dwellMs: 400 },
    { type: 'dom-mutation-burst', tsEpochMs: 4450, sessionSeq: 42, pageSeq: 13, sourceSeq: 3, pageInstanceId: 'p1', recordCount: 4, targetRefs: ['thumb'], addedRefs: ['previewControl'], attributes: {} },
    { type: 'dom-hover-leave', tsEpochMs: 4800, sessionSeq: 43, pageSeq: 14, sourceSeq: 4, pageInstanceId: 'p1', targetRef: 'thumb', dwellMs: 800 }
  ]
};

const result = Windows.buildActionWindows(raw);
assert.strictEqual(result.actionWindowVersion, '0.1.0');
...
// Chronology must be based on capture time, not persistence sequence.
const scrambled = [
