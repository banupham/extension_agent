...
function eventRef(event) {
  ...
  if (event.type === 'pointer') Object.assign(out, {
    ...
    button: finiteOrNull(event.button),
    buttons: finiteOrNull(event.buttons),
...
function boundedPointerFacts(ordered, anchor, startTs, endTs, maxSamples = 120) {
  const points = ordered.filter(event => event.type === 'pointer' && sameContext(anchor, event) && Number(event.tsEpochMs || 0) >= startTs && Number(event.tsEpochMs || 0) <= endTs);
  if (points.length <= maxSamples) return points.map(eventRef);
  return points.slice(points.length - maxSamples).map(eventRef);
}
...
function clickWindows(ordered, options, descriptorIndex) {
  ...
  return ordered.filter(event => event.type === 'dom-click').map((anchor, index) => {
    ...
    return {
      ...
      actionType: classifyClickAction(target),
...
function formWindows(ordered, options, descriptorIndex) {
  const beforeMs = Number(options.beforeMs || 800);
  const afterMs = Number(options.afterMs || 1000);
  ...
  return anchors.map((anchor, index) => {
    ...
    return {
      actionWindowVersion: DERIVED_VERSION,
      actionId: `form-${index + 1}-${Number(anchor.sessionSeq || anchor.pageSeq || 0)}`,
      actionType,
      anchorTsEpochMs: Number(anchor.tsEpochMs || 0),
      context: { tabId: anchor.tabId ?? null, frameId: anchor.frameId ?? null, pageInstanceId: anchor.pageInstanceId || null },
      target: targetContext(anchor, descriptorIndex),
      before: windowEvents.filter(e => Number(e.tsEpochMs || 0) < Number(anchor.tsEpochMs || 0)).map(eventRef),
      action: eventRef(anchor),
      after: windowEvents.filter(e => Number(e.tsEpochMs || 0) > Number(anchor.tsEpochMs || 0)).map(eventRef),
      outcome: outcomeFrom(windowEvents, Number(anchor.tsEpochMs || 0))
    };
  }).filter(Boolean);
}
...
function dragWindows(ordered, options, descriptorIndex) {
  ...
  for (const event of ordered) {
    ...
    windows.push({
      ...
      actionType: 'drag',
      anchorTsEpochMs: Number(state.down.tsEpochMs || 0),
      ...
      before: [],
      action: {
        startTsEpochMs: Number(state.down.tsEpochMs || 0),
        endTsEpochMs: Number(event.tsEpochMs || 0),
        durationMs,
        distancePx: Math.round(distancePx * 1000) / 1000,
        start: { x: finiteOrNull(state.down.x), y: finiteOrNull(state.down.y) },
        end: { x: finiteOrNull(event.x), y: finiteOrNull(event.y) },
        pointerId: finiteOrNull(state.down.pointerId),
        points: state.events.map(eventRef)
      },
...
function scrollWindows(ordered, options, descriptorIndex) {
  ...
  return groups.map((group, index) => ({
    ...
    action: {
      ...
      endTsEpochMs: Number(group.events.at(-1)?.tsEpochMs || 0),
...
function hoverWindows(raw, ordered, descriptorIndex, options = {}) {
  ...
  const approachMs = Number(options.hoverApproachMs || 1200);
  const leaveTailMs = Number(options.hoverLeaveTailMs || 500);
  const maxPointerSamples = Number(options.hoverMaxPointerSamples || 120);
  let filteredNoiseCount = 0;
  let hoverWithApproachCount = 0;
  let hoverWithLeaveTrajectoryCount = 0;
  const windows = [];
  ...
  for (let index = 0; index < (derived.hoverActions || []).length; index += 1) {
    ...
    const anchor = candidates.find(event => Math.abs(Number(event.tsEpochMs || 0) - Number(action.startedAtEpochMs || 0)) <= 10)
      || candidates[0]
      || { pageInstanceId: action.pageInstanceId, targetRef: action.targetRef, tsEpochMs: action.startedAtEpochMs };
    const target = targetContext(anchor, descriptorIndex);
    ...
    const startTs = Number(action.startedAtEpochMs || 0);
    const endTs = Number(action.endedAtEpochMs || startTs);
    const before = boundedPointerFacts(ordered, anchor, startTs - approachMs, Math.max(0, startTs - 0.001), maxPointerSamples);
    const after = boundedPointerFacts(ordered, anchor, endTs + 0.001, endTs + leaveTailMs, maxPointerSamples);
    if (before.length) hoverWithApproachCount += 1;
    if (after.length) hoverWithLeaveTrajectoryCount += 1;
  ...
  return { windows, filteredNoiseCount, hoverWithApproachCount, hoverWithLeaveTrajectoryCount };
}
...
function buildActionWindows(raw, options = {}) {
  ...
  const windows = [
    ...
    ...formWindows(ordered, options, descriptorIndex),
  ...
  return {
    ...
    derivation: {
      targetLabelEnrichment: 'resolvedTargetDescriptor -> snapshot/index -> targetDescriptor -> raw descendant',
      ...
      hoverPointerApproach: {
        beforeMs: Number(options.hoverApproachMs || 1200),
        leaveTailMs: Number(options.hoverLeaveTailMs || 500),
        maxPointerSamples: Number(options.hoverMaxPointerSamples || 120),
        withApproachCount: hover.hoverWithApproachCount,
        withLeaveTrajectoryCount: hover.hoverWithLeaveTrajectoryCount
      },
...
function main(argv = process.argv.slice(2)) {
  ...
  console.log(JSON.stringify({
    ...
    filteredHoverNoiseCount: result.derivation.filteredHoverNoiseCount,
    hoverWithApproachCount: result.derivation.hoverPointerApproach.withApproachCount
  }, null, 2));
}
