...
function taskHasAll(task, candidates) {
  const set = new Set(words(task?.instruction));
  return candidates.every(word => set.has(word));
}
function taskRequestsDoubleClick(task) {
  const tokens = new Set(words(task?.instruction));
  return (tokens.has('double') && tokens.has('click')) || (tokens.has('nhap') && tokens.has('dup')) || (tokens.has('click') && tokens.has('dup'));
}
...
function transitionById(review, transitionId) {
  return (Array.isArray(review?.transitions) ? review.transitions : []).find(item => String(item?.transitionId || '') === String(transitionId || '')) || null;
}
...
function observationUrl(observation) { return String(observation?.url || observation?.page?.url || '').trim(); }
...
function normalizedMediaState(value) {
  ...
  return { paused: value.paused === true, muted: value.muted === true, volume: finiteOrNull(value.volume), currentTime: finiteOrNull(value.currentTime), duration: finiteOrNull(value.duration), playbackRate: finiteOrNull(value.playbackRate) };
}
function observationFingerprint(observation) {
  const elements = observationElements(observation).map(element => ({
    ...
    rangeValue: finiteOrNull(element?.rangeValue), rangeMin: finiteOrNull(element?.rangeMin), rangeMax: finiteOrNull(element?.rangeMax),
    mediaState: normalizedMediaState(element?.mediaState)
  })).sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b)));
  return JSON.stringify({ url: observationUrl(observation), elements, pageSignals: observation?.pageSignals || observation?.page?.pageSignals || {} });
}
function observableSemanticStateChanged(transition) { return observationFingerprint(transition?.strategyObservationBefore) !== observationFingerprint(transition?.strategyObservationAfter); }
function targetForTransition(transition) {
  const ref = typeof transition?.rawAction?.targetRef === 'string' && transition.rawAction.targetRef.trim() ? transition.rawAction.targetRef.trim() : null;
  if (!ref) return { ref: null, element: null };
  const element = observationElements(transition?.strategyObservationBefore).find(item => item?.ref === ref || item?.elementRef === ref || item?.targetRef === ref) || null;
  return { ref, element };
}
function targetAfterForTransition(transition, ref = null) {
  const targetRef = ref || transition?.rawAction?.targetRef || null;
  if (!targetRef) return null;
  return observationElements(transition?.strategyObservationAfter).find(item => item?.ref === targetRef || item?.elementRef === targetRef || item?.targetRef === targetRef) || null;
}
function semanticTarget(element) {
  if (!element) return null;
  return {
    label: typeof element.label === 'string' ? element.label : null, role: typeof element.role === 'string' ? element.role : null,
    tag: typeof element.tag === 'string' ? element.tag : null, inputType: typeof element.inputType === 'string' ? element.inputType : null,
    editable: element.editable === true, draggable: element.draggable === true, enabled: element.enabled !== false,
    visible: element.visible !== false && element.rendered !== false, checked: typeof element.checked === 'boolean' ? element.checked : null,
    selected: typeof element.selected === 'boolean' ? element.selected : null,
    selectedIndex: Number.isInteger(Number(element.selectedIndex)) ? Number(element.selectedIndex) : null,
    rangeValue: finiteOrNull(element.rangeValue), rangeMin: finiteOrNull(element.rangeMin), rangeMax: finiteOrNull(element.rangeMax), rangeStep: finiteOrNull(element.rangeStep),
    mediaState: normalizedMediaState(element.mediaState)
  };
}
function safeAction(type, targetRef, args = {}, intent = null) {
  return validateAgentAction({ contractVersion: '0.1.0', type, targetRef: targetRef || null, args, intent: intent || `ambiguity-resolution:${type}`, expectedOutcome: {} });
}
function controlKey(rawAction = {}) {
  const candidates = [rawAction.key, rawAction.code, rawAction.operation].filter(value => typeof value === 'string').map(value => value.toLowerCase());
...
function targetKind(element) {
  const role = String(element?.role || '').toLowerCase(), tag = String(element?.tag || '').toLowerCase(), inputType = String(element?.inputType || '').toLowerCase();
...
function targetLabelWords(target) { return new Set(words(target?.label)); }
...
function normalizedRangeRequest(type, rawAction = {}) {
  const value = finiteOrNull(rawAction.rangeValue), min = finiteOrNull(rawAction.rangeMin), max = finiteOrNull(rawAction.rangeMax);
  if (value == null) return null;
  if ((type === 'setVolume' || type === 'seek') && min === 0 && max === 100) return value;
  return value;
}
function resolved(transitionId, hint, action, target, reasonCode) {
  return { transitionId, sourceHint: hint || null, status: 'resolved-semantic-action', semanticActionType: action.type, suggestedAction: action, exclusionReason: null, reasonCode, semanticTarget: target, requiresHumanConfirmation: true, autoTrainEligible: false };
}
function noise(transitionId, hint, target, reasonCode) {
  return { transitionId, sourceHint: hint || null, status: 'capture-noise', semanticActionType: null, suggestedAction: null, exclusionReason: reasonCode, reasonCode, semanticTarget: target, requiresHumanConfirmation: true, autoTrainEligible: false };
}
function unresolved(transitionId, hint, target, reasonCode, semanticActionType = null) {
  return { transitionId, sourceHint: hint || null, status: 'needs-human-review', semanticActionType, suggestedAction: null, exclusionReason: null, reasonCode, semanticTarget: target, requiresHumanConfirmation: true, autoTrainEligible: false };
}
function resolveFormChange({ transitionId, hint, transition, task, ref, element, target }) {
  const raw = transition?.rawAction || {}, afterElement = targetAfterForTransition(transition, ref), afterTarget = semanticTarget(afterElement) || target;
  const kind = targetKind(element || target || afterTarget || {});
  if ((kind === 'toggle' || kind === 'radio') && ref) {
    const desired = typeof raw.checked === 'boolean' ? raw.checked : typeof afterTarget?.checked === 'boolean' ? afterTarget.checked : null;
    if (desired == null) return unresolved(transitionId, hint, target, 'checkable_desired_state_missing', 'setChecked');
    return resolved(transitionId, hint, safeAction('setChecked', ref, { value: desired }, 'captured-checkable-state-change'), target, 'captured_checkable_desired_state');
  }
  if (kind === 'select' && ref) {
...
function resolveMediaAction({ transitionId, hint, transition, ref, target }) {
  const raw = transition?.rawAction || {}, operation = String(raw.operation || '').trim();
  if (!ref) return unresolved(transitionId, hint, target, 'media_target_required');
  if (['play', 'pause', 'mute', 'unmute'].includes(operation)) return resolved(transitionId, hint, safeAction(operation, ref, {}, `captured-media-${operation}`), target, 'captured_media_operation');
  if (operation === 'changePlaybackRate' && finiteOrNull(raw.playbackRate) != null) return resolved(transitionId, hint, safeAction('changePlaybackRate', ref, { value: Number(raw.playbackRate) }, 'captured-playback-rate'), target, 'captured_playback_rate');
  return unresolved(transitionId, hint, target, operation ? `unsupported_media_operation_${operation}` : 'media_operation_missing');
}
function resolveWaitAction({ transitionId, hint, transition, task, target }) {
  const raw = transition?.rawAction || {};
  const waitedMs = finiteOrNull(raw.waitedMs);
  const explicitWait = taskMentions(task, ['wait', 'cho', 'ready', 'san', 'available']);
  if (!explicitWait) return noise(transitionId, hint, target, 'delayed_change_not_explicitly_part_of_task');
  if (waitedMs == null || waitedMs < MIN_SEMANTIC_WAIT_MS) return unresolved(transitionId, hint, target, 'semantic_wait_duration_insufficient', 'waitAndObserve');
  if (!observableSemanticStateChanged(transition)) return noise(transitionId, hint, target, 'wait_without_semantic_state_change_how_not_strategy');
  return resolved(transitionId, hint, safeAction('waitAndObserve', null, {}, 'captured-delayed-semantic-change'), target, 'explicit_wait_with_delayed_semantic_state_change');
}
function resolveBrowserLifecycle({ transitionId, hint, transition, task, target }) {
  const raw = transition?.rawAction || {};
  let operation = String(raw.operation || '').trim();
  const kind = String(raw.kind || '').trim();
  if (!operation && kind.startsWith('tab-')) operation = kind.slice(4);
  const tokens = new Set(words(task?.instruction));
  const has = word => tokens.has(word);
  if (operation === 'openNewTab') {
    if (!(has('tab') && (has('open') || has('mo') || has('new') || has('moi')))) return unresolved(transitionId, hint, target, 'open_new_tab_not_explicit_in_task', 'openNewTab');
    const url = observationUrl(transition?.strategyObservationAfter);
    if (!/^https?:\/\//i.test(url)) return unresolved(transitionId, hint, target, 'open_new_tab_safe_url_missing', 'openNewTab');
    return resolved(transitionId, hint, safeAction('openNewTab', null, { url }, 'captured-browser-open-new-tab'), target, 'captured_new_tab_with_safe_observed_url');
  }
  if (operation === 'switchTab') {
    if (!(has('tab') && (has('switch') || has('chuyen') || has('quay') || has('return')))) return unresolved(transitionId, hint, target, 'switch_tab_not_explicit_in_task', 'switchTab');
  ...
  if (operation === 'back') {
    ...
    return resolved(transitionId, hint, safeAction('back', null, {}, 'captured-browser-back'), target, 'captured_explicit_browser_back');
  }
  if (operation === 'forward') {
    if (!(has('forward') || has('toi'))) return unresolved(transitionId, hint, target, 'browser_forward_not_explicit_in_task', 'forward');
    return resolved(transitionId, hint, safeAction('forward', null, {}, 'captured-browser-forward'), target, 'captured_explicit_browser_forward');
  }
  if (operation === 'reload') {
    if (!(has('reload') || taskHasAll(task, ['tai', 'lai']) || taskHasAll(task, ['lam', 'moi']))) return unresolved(transitionId, hint, target, 'browser_reload_not_explicit_in_task', 'reload');
    return resolved(transitionId, hint, safeAction('reload', null, {}, 'captured-browser-reload'), target, 'captured_explicit_browser_reload');
  }
  return unresolved(transitionId, hint, target, operation ? `unsupported_browser_lifecycle_${operation}` : 'browser_lifecycle_operation_missing');
}
function resolveAmbiguousTransition({ proposal, transition, task }) {
  ...
  if (hint === 'form-control-click-review-required') return noise(transitionId, hint, target, 'form_control_surface_click_how_not_strategy');
  if (hint === 'click' && taskRequestsDoubleClick(task)) return noise(transitionId, hint, target, 'component_click_of_explicit_double_click_how_not_strategy');
  if (hint === 'doubleClick') {
  ...
  if (hint === 'hoverAndObserve' || hint === 'hover-review-required') {
    ...
    if (!observableSemanticStateChanged(transition)) return noise(transitionId, hint, target, 'incidental_hover_without_semantic_state_change_how_not_strategy');
  ...
  if (hint === 'drag' || hint === 'drag-review-required') {
    const destinationRef = typeof transition?.rawAction?.destinationRef === 'string' ? transition.rawAction.destinationRef.trim() : '';
  ...
  if (hint === 'form-control-review-required' || hint === 'media-range-review-required') return resolveFormChange({ transitionId, hint, transition, task, ref, element, target });
  if (hint === 'media-action-review-required') return resolveMediaAction({ transitionId, hint, transition, ref, target });
  if (hint === 'waitAndObserve-review-required') return resolveWaitAction({ transitionId, hint, transition, task, target });
  if (hint === 'tab-lifecycle-review-required') return resolveBrowserLifecycle({ transitionId, hint, transition, task, target });
  if (hint === 'submit-review-required') {
    if (!ref) return unresolved(transitionId, hint, target, 'submit_target_required', 'submit');
    if (!taskMentions(task, ['submit', 'send', 'search', 'gui', 'luu'])) return unresolved(transitionId, hint, target, 'submit_not_explicit_in_task', 'submit');
    return resolved(transitionId, hint, safeAction('submit', ref, {}, 'captured-semantic-submit-control'), target, 'captured_explicit_submit_control');
  }
  if (hint === 'text-action-review-required') {
    const kind = targetKind(element || target || {});
    if (kind === 'editable') return unresolved(transitionId, hint, target, 'text_content_intentionally_redacted_requires_human_review', 'typeText');
  ...
  if (hint === 'keyboard-action-review-required') {
    ...
    if (key === 'Enter' && ref && taskMentions(task, ['submit', 'search', 'send', 'go', 'tim', 'gui', 'nhan'])) return resolved(transitionId, hint, safeAction('submit', ref, {}, 'semantic-submit-via-enter'), target, 'enter_key_semantic_submit');
    if (key === 'Escape' && ref && taskMentions(task, ['dismiss', 'close', 'cancel', 'dong', 'huy'])) return resolved(transitionId, hint, safeAction('dismiss', ref, {}, 'semantic-dismiss-via-escape'), target, 'escape_key_semantic_dismiss');
  ...
  if (hint === 'scroll-direction-review-required') {
    ...
    if (!taskMentions(task, ['scroll', 'cuon'])) return noise(transitionId, hint, target, 'incidental_scroll_how_not_strategy');
    if (direction === 'horizontal') return resolved(transitionId, hint, safeAction('scrollHorizontal', null), target, 'task_explicit_horizontal_scroll');
    if (direction === 'vertical') return resolved(transitionId, hint, safeAction('scrollVertical', null), target, 'task_explicit_vertical_scroll');
...
function resolutionForItem(packItem, triageItem, sourceReview) {
  const scoreByTransition = new Map((Array.isArray(triageItem?.transitions) ? triageItem.transitions : []).map(item => [String(item?.transitionId || ''), item]));
  const resolutions = [];
  ...
  return { episodeId: packItem?.episodeId || null, task: packItem?.task || null, finalOutcomeStatus: packItem?.finalOutcomeStatus || null, ambiguousTransitionCount: resolutions.length, resolvedSemanticActionCount: resolvedCount, captureNoiseCount, unresolvedHumanReviewCount: unresolvedCount, allAmbiguityResolvedForApprovalAid: resolutions.length > 0 && unresolvedCount === 0, resolutions };
}
function markdownFor(result) {
  const lines = ['# Strategy ambiguity resolution', '', `Episodes: ${result.episodeCount}`, `Ambiguous transitions: ${result.ambiguousTransitionCount}`, `Resolved semantic actions: ${result.resolvedSemanticActionCount}`, `Capture noise: ${result.captureNoiseCount}`, `Still needs human review: ${result.unresolvedHumanReviewCount}`, `Episodes fully resolved as review aids: ${result.fullyResolvedEpisodeCount}`, '', '> Resolution outputs are review aids only. They never count as human verification or automatic training eligibility.', ''];
...
function resolveReviewPack(packFile, triageFile, outputDir) {
  const fullPack = path.resolve(packFile), fullTriage = path.resolve(triageFile), pack = readJson(fullPack), triage = readJson(fullTriage);
  ...
  const jsonFile = path.join(outDir, 'ambiguity-resolution.json'), markdownFile = path.join(outDir, 'ambiguity-resolution.md');
  fs.writeFileSync(jsonFile, `${JSON.stringify(result, null, 2)}\n`, 'utf8'); fs.writeFileSync(markdownFile, markdownFor(result), 'utf8');
  return { result, jsonFile, markdownFile };
}
function parseArgs(argv) { const out = {}; for (let i = 0; i < argv.length; i += 1) { const token = argv[i]; if (!token.startsWith('--')) continue; const key = token.slice(2), next = argv[i + 1]; if (!next || next.startsWith('--')) out[key] = true; else { out[key] = next; i += 1; } } return out; }
...
module.exports = { AMBIGUITY_RESOLVER_VERSION, MIN_SEMANTIC_WAIT_MS, words, taskMentions, taskHasAll, taskRequestsDoubleClick, taskDeclaredSelection, taskDeclaredPlaybackRate, transitionById, observationElements, observationUrl, observationFingerprint, observableSemanticStateChanged, targetForTransition, targetAfterForTransition, semanticTarget, safeAction, controlKey, scrollDirection, targetKind, mediaRangeActionType, resolveFormChange, resolveMediaAction, resolveWaitAction, resolveBrowserLifecycle, resolveAmbiguousTransition, resolutionForItem, markdownFor, resolveReviewPack, parseArgs, main };
