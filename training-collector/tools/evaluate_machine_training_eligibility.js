...
function observationFingerprint(observation) {
  ...
  const pageSignals = observation?.pageSignals && typeof observation.pageSignals === 'object'
    ...
    : (observation?.page?.pageSignals && typeof observation.page.pageSignals === 'object'
        ? observation.page.pageSignals
        : {});
...
function semanticStateChanged(transition) {
...
function explicitSuccessCriteriaVerification(review) {
  ...
  if (!transitions.length) {
    return {
      status: 'contradicted',
      source: 'explicit-success-criteria',
      confidence: 1,
      reasons: ['episode_has_no_transitions'],
      outcome: null
    };
  }
  ...
  const result = evaluateGoal({
    task: review.task,
    execution: { ok: allActionsSucceeded },
    before: transitions[0]?.strategyObservationBefore || null,
    after: transitions.at(-1)?.strategyObservationAfter || null,
    beforeBrowserContext: null,
    afterBrowserContext: null
  });
  return {
    status: result?.taskSucceeded === true ? 'verified' : 'contradicted',
    source: 'explicit-success-criteria',
    confidence: 1,
    reasons: result?.taskSucceeded === true ? ['success_criteria_satisfied'] : ['success_criteria_not_satisfied'],
    outcome: result
  };
}
...
function declaredMachineSignalVerification(review) {
  ...
  if (!after) {
    return {
      status: 'contradicted',
      source: 'declared-machine-success-signal',
      confidence: 1,
      reasons: ['final_observation_missing']
    };
  }
  ...
  if (typeof expected.title === 'string') matched = actualTitle === expected.title;
    ...
    matched = Object.prototype.hasOwnProperty.call(signals, expected.pageSignalKey) &&
      signals[expected.pageSignalKey] === expected.pageSignalValue;
  }
  return {
    status: matched ? 'verified' : 'contradicted',
    source: 'declared-machine-success-signal',
    confidence: 1,
    reasons: [matched ? 'machine_success_signal_satisfied' : 'machine_success_signal_not_satisfied']
  };
}
...
function genericOutcomeSupport(review) {
  ...
  if (finalStatus !== 'success') {
    return {
      status: 'contradicted',
      source: 'captured-demonstration',
      confidence: 1,
      reasons: ['final_outcome_not_success']
    };
  }
  if (!transitions.length) {
    return {
  ...
  if (transitions.some(item => item?.outcome?.actionSucceeded === false)) {
    return {
      status: 'contradicted',
      source: 'captured-demonstration',
      confidence: 0.95,
      reasons: ['captured_action_failure_present']
    };
  }
  const changed = transitions.filter(semanticStateChanged).length;
  return {
    status: changed > 0 ? 'supported' : 'unverified',
    source: 'captured-semantic-change',
    confidence: changed > 0 ? 0.7 : 0.4,
    reasons: [changed > 0 ? 'semantic_state_change_observed' : 'no_independent_semantic_state_change_observed'],
    semanticStateChangedTransitionCount: changed
  };
}

function verifyTaskOutcome(review) {
  const explicit = explicitSuccessCriteriaVerification(review);
...
function classifyEpisode({ queueItem, packItem, resolutionItem, candidate, blockedCandidate, candidatePolicy, review }) {
  ...
  if (!reasons.length && outcomeVerification.status === 'verified') {
    return {
      episodeId,
      status: 'accept',
...
function evaluateMachineTrainingEligibility({ manifest, reviewPack, resolution, candidates } = {}) {
  ...
  for (const queueItem of queue) {
    ...
    let readError = null;
    try {
      if (sourceFile && fs.existsSync(sourceFile)) review = readJson(sourceFile);
      else readError = 'review_source_missing';
    } catch (error) {
    ...
    if (!review) {
      items.push({
        episodeId,
        status: 'reject',
        reasons: [readError || 'review_source_unreadable'],
        outcomeVerification: { status: 'unverified', source: 'none', confidence: 0, reasons: [readError || 'review_source_unreadable'] },
        semantic: { ok: false, reasons: ['review_source_unreadable'] }
      });
      continue;
    }
    items.push(classifyEpisode({
      queueItem,
      packItem,
  ...
  const counts = {
    accept: items.filter(item => item.status === 'accept').length,
    quarantine: items.filter(item => item.status === 'quarantine').length,
    reject: items.filter(item => item.status === 'reject').length
  };
  return {
...
module.exports = {
  ...
  classifyEpisode,
  evaluateMachineTrainingEligibility
};
