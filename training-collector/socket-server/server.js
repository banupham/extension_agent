...
function broadcast(payload) {
  for (const ws of socketClients) send(ws, payload);
}
function queuePipeline(job) {
  const next = pipelineChain.then(job);
  pipelineChain = next.catch(error => console.error('[collector] pipeline error', error));
  return next;
}
function strategyPipelineTools() {
  return require('../tools/prepare_incremental_strategy_learning.js');
}
...
async function listMachineReceipts() {
  ...
  for (const name of names) {
    try { rows.push(JSON.parse(await fsp.readFile(path.join(RECEIPT_DIR, name), 'utf8'))); } catch {}
  }
  return rows;
}
...
async function pipelineStatus() {
  return publicPipelineState(await refreshPipelineSummary());
}
...
function reviewSafety(review) {
  ...
  if (String(review?.reviewExportVersion || '').trim() !== '0.1.0') reasons.push('unsupported_review_export_version');
...
async function classifyReviewFile(file) {
  ...
  try {
    ...
    const prepared = tools.prepareIncrementalStrategyLearning({
      reviewRoot: file,
      baseDatasetDir: BASE_DATASET_DIR,
      outputDir: work
    });
...
async function acceptedUnassignedReceipts() {
  const state = await loadPipelineState();
  const assigned = new Set(Array.isArray(state.pendingCandidate?.episodeIds) ? state.pendingCandidate.episodeIds : []);
  return (await listMachineReceipts())
    .filter(row => row?.status === 'accept' && !assigned.has(row.episodeId))
...
async function maybeBuildCandidateUnlocked() {
  ...
  try {
    ...
    if (Number(prepared.bundle?.machineAcceptEpisodeCount || 0) < PIPELINE_BATCH_THRESHOLD) {
      throw new Error(`candidate_batch_recheck_below_threshold:${prepared.bundle?.machineAcceptEpisodeCount || 0}/${PIPELINE_BATCH_THRESHOLD}`);
    }

    let finalized = tools.finalizeMachineAcceptedStrategyLearning(prepared, {
      baseDatasetDir: BASE_DATASET_DIR,
    ...
    if (PIPELINE_AUTO_PROTECT) {
      try {
        ...
        await atomicJson(path.join(outputDir, 'candidate-protection-error.json'), {
          failedAt: nowIso(),
    ...
    const acceptedEpisodeIds = Array.isArray(prepared.machineEligibility?.machineAcceptEpisodeIds)
      ? prepared.machineEligibility.machineAcceptEpisodeIds
      : selected.map(row => row.episodeId);
    const candidateStatus = protectionError
      ? 'candidate-protection-error'
      : String(finalized.finalManifest?.status || 'candidate-awaiting-runtime-protection');
    ...
    pipelineState.lastResult = {
      episodeId: null,
      status: candidateStatus,
      reasons: protectionError ? ['candidate_protection_error'] : ['candidate_batch_built'],
      at: nowIso()
    };
    ...
    stateAfterError.lastResult = {
      episodeId: null,
      status: 'candidate-build-error',
      reasons: ['candidate_build_error'],
      at: nowIso()
    };
    ...
    await fsp.rm(inputDir, { recursive: true, force: true }).catch(() => {});
  }
}
...
function scheduleReviewProcessing(file) {
  ...
  queuePipeline(async () => {
    try {
      ...
      broadcast({
        type: 'task-episode-review-result',
      ...
      processingEpisodeIds.delete(episodeId);
    }
  }).catch(() => {});
}
...
async function receiveTaskReview(ws, message) {
  ...
  if (!safety.ok) {
    ...
    const pipeline = await pipelineStatus();
    send(ws, {
      type: 'task-episode-review-ack',
      protocol: 'training-collector-v1',
      episodeId: safety.episodeId || String(message?.episodeId || ''),
      persisted: false,
      permanent: true,
      status: receipt?.status || 'reject',
  ...
  if (fs.existsSync(file)) {
    ...
    if (existingDigest !== incomingDigest) {
      send(ws, {
        ...
        conflict: true,
        status: 'reject',
        reasons: ['episode_review_digest_conflict'],
        pipeline: await pipelineStatus()
      });
      return;
    }
    duplicate = true;
  ...
  send(ws, {
    ...
    permanent: false,
    duplicate,
    status: priorReceipt?.status || 'queued',
    reviewSha256: incomingDigest,
    pipeline: await pipelineStatus()
  });
...
async function finalizeSession(state, details = {}) {
  await queueState(state, async () => {
    ...
    if (state.status === 'closed' && expectedLastSeq <= state.lastSeq) return;
    state.status = 'closed';
    ...
    ws.on('message', async data => {
      ...
      try {
        ...
        if (message.type === 'client-hello') {
          send(ws, { type: 'client-hello-ack', protocol: 'training-collector-v1', at: nowIso() });
          return send(ws, { type: 'pipeline-status', protocol: 'training-collector-v1', pipeline: await pipelineStatus() });
        }
